...
export default function ProgressBar({ active, onChange, collapsed, setCollapsed, userLogo }) {
  ...
  return (
    <div
      className="fixed top-0 left-0 h-screen flex flex-col justify-between border-r transition-all duration-300 z-50 font-plex-hebrew bg-[#EEF2FF] overflow-y-auto custom-scrollbar"
      style={{
        width: collapsed ? "101px" : "235px",
        // Responsive Padding: Mobile (16px/1rem) -> Desktop (24px/1.5rem) logic handled via inline style or classes
        // Keeping your explicit logic but allowing shrink
        padding: "24px 16px",
    ...
    >
      ...
      <div className="flex flex-col items-center w-full">
        ...
        <button
          ...
          className="bg-white rounded-xl shadow-sm flex items-center justify-center hover:bg-gray-50 transition-colors mb-6 md:mb-10" // Responsive Margin Bottom
          style={{
            width: "40px",    
            height: "40px",   
            padding: "5px 8px",
            // marginBottom handled by Tailwind classes above for responsiveness
          }}
        >
        ...
        {/* Menu Items List */}
        {/* Responsive Gap: gap-5 on mobile, gap-10 (40px) on desktop */}
        <div className="flex flex-col w-full gap-5 md:gap-[40px]"> 
          {menu.map((m) => (
            <button
              key={m.id}
            ...
            >
              ...
              <span
                ...
                style={{
                    ...
                    fontWeight: "400",
                    // Responsive Font Size logic could be added here if needed, but keeping 14px as requested
                    fontSize: "14px",
      ...
      {/* --- BOTTOM SECTION --- */}
      <div 
        className="flex flex-col items-center w-full mt-6 md:mt-0" 
        style={{ gap: "20px" }}
      >
